...
export default class ListenerCollection {
   ...
   */
  add(key: string, context: any, listener: Function) {
  ...
  fireAndJoinResults(...rest): Promise<boolean> {
    return this.fire.apply(this, ...rest).then(function (results) {
      ...
      if (
        successes.length &&
        successes.length + dontCares.length === results.length
      ) {
        // overrided pass
        return true;
      }
      ...
      if (
        failures.length &&
        failures.length + dontCares.length === results.length
      ) {
        return false;
      }
  ...
  fire(...args): Promise<any> {
    ...
    //   (listener as any).fn.apply((listener as any).context, currentArgs);
    // }
