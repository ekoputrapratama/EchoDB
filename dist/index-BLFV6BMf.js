...
class Table {
  constructor(name, provider) {
    this.name = name;
    this.collection = provider.collection(name);
  }
  select(q, ...args) {
    if (args.length === 1) {
...
function validateSchemaProps$1(keys2) {
  keys2.forEach((key) => {
    if (!validSchemaProperty$1.includes(key)) {
      throw new Error(`property ${key} is not a known property`);
    }
  });
}
function addKey(schema, column, unique) {
  if (unique) return schema + `&${column},`;
  else return schema + `${column},`;
}
function addPrimaryKey(schema, column, autoIncrement) {
  if (autoIncrement) return schema + `++${column},`;
  else return schema + `${column},`;
}
function objectToSchemaString(columns) {
  let schema = "";
  ...
  for (const column in columns) {
    const typeOrProps = columns[column];
    if (!types.includes(typeOrProps) && isObject$1(typeOrProps)) {
      const keys2 = Object.keys(typeOrProps);
      validateSchemaProps$1(keys2);
      if (keys2.includes("type") && !types.includes(typeOrProps["type"]))
        throw new Error(`type ${typeOrProps["type"]} is not a known type`);
      if (keys2.includes("index")) {
        indexes.push(column);
      }
      if (keys2.includes("primaryKey") || keys2.includes("autoIncrement")) {
        if (havePrimaryKey)
          throw new Error(`only 1 primaryKey can be defined for each schema`);
        if (keys2.includes("autoIncrement") && !keys2.includes("primaryKey"))
          throw new Error(`only primaryKey can use autoIncrement property`);
        schema = addPrimaryKey(schema, column, keys2.includes("autoIncrement"));
        primaryKeyColumn = column;
        if (keys2.includes("primaryKey")) havePrimaryKey = true;
        if (keys2.includes("autoIncrement")) autoIncrement = true;
        continue;
      }
      if (keys2.includes("unique")) {
...
function buildCompoundIndex(schema, primaryKeyColumn, indexes) {
...
function reorderSchema(schema, primaryKeyColumn) {
  const splitted = schema.split(",");
  let i = 0;
...
class EchoDB extends EventEmitter {
  constructor(options) {
    super();
    this.options = options;
    this.provider = options.adapter;
    if (options.sync && options.sync.adapter) {
  ...
  table(name) {
    return new Table(name, this.provider);
  }
  close() {
    this.provider.close();
    if (this.syncAdapter) {
...
function validateSchemaProps(keys2) {
  keys2.forEach((key) => {
...
function validateData(data, schema) {
  ...
  Object.keys(data).forEach((key) => {
    ...
    switch (type) {
      case String:
        if (!isString$1(data[key])) {
          throw new Error(
            "column " + key + " expected to have type of string but " + typeof data[key] + " given."
          );
        }
        break;
      case Number:
        if (!isNumber$1(data[key])) {
          throw new Error(
            "column " + key + " expected to have type of number but " + typeof data[key] + " given."
          );
        }
        break;
      case Array:
        if (!Array.isArray(data[key])) {
          throw new Error(
            "column " + key + " expected to have type of array but " + typeof data[key] + " given."
          );
        }
        break;
      case Date:
        if (!hooks.isDate(data[key])) {
          throw new Error(
            "column " + key + " expected to have type of date but " + typeof data[key] + " given."
          );
        }
        break;
      case Object:
        if (!isObject$1(data[key])) {
          throw new Error(
            "column " + key + " expected to have type of object but " + typeof data[key] + " given."
          );
        }
        break;
    }
...
function wrapObject(obj, collection) {
  let doc = obj;
  if (isSnapshot(doc)) {
...
function createWorkerImportScript(scriptFn) {
...
function isNodejs() {
  return (
    ...
    new Function("try {return this===global;}catch(e){return false;}") && typeof process === "object" && typeof process.versions === "object" && typeof process.versions.node !== "undefined"
  );
}
function isBrowser() {
  return new Function(
...
function requireEachLimit() {
  ...
  (function(module2, exports) {
    ...
    exports.default = (0, _awaitify2.default)(eachLimit2, 4);
    module2.exports = exports.default;
...
function requireEachSeries() {
  ...
  (function(module2, exports) {
    ...
    function eachSeries2(coll, iteratee, callback) {
      return (0, _eachLimit2.default)(coll, 1, iteratee, callback);
    }
    exports.default = (0, _awaitify2.default)(eachSeries2, 3);
    module2.exports = exports.default;
  })(eachSeries, eachSeries.exports);
  return eachSeries.exports;
}
var eachSeriesExports = requireEachSeries();
const forEachSeries = /* @__PURE__ */ getDefaultExportFromCjs(eachSeriesExports);
/*!
...
if (typeof global !== "undefined") {
  global.EchoDB = module.exports;
}
export {
  CursorProvider as C,
  DatabaseProvider as D,
  EventEmitter as E,
  ListenerCollection as L,
  SyncProvider as S,
  Table as T,
  EchoDB as a,
  CollectionProvider as b,
  Schema as c,
  validSchemaProperty as d,
  validateSchemaProps as e,
  validatePopulate as f,
  validatePopulates as g,
  validateData as h,
  isInWorker as i,
  isFirestoreTimestamp as j,
  createWorker as k,
  loadNetworkProvider as l,
  createWorkerImportScript as m,
  normalize as n,
  isDate as o,
  isWorkerAvailable as p,
  isNodejs as q,
  isBrowser as r,
  workerScriptPath as s,
  toRawObject as t,
  isNestedQuery as u,
  validateSchema as v,
  wrapObject as w
